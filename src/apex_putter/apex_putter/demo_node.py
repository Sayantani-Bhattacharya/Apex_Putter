...
class DemoNode(Node):
    def __init__(self):
        ...
        self.trajectory_calculator = BallTrajectoryCalculator(self.ball_position,
                                                              self.hole_position)
        ...
        # Markers
        self.ball_marker_pub = self.create_publisher(Marker,
                                                     'ball_marker',
                                                     10)
        self.hole_marker_pub = self.create_publisher(Marker,
                                                     'hole_marker',
                                                     10)
        ...
        # Services
        self.sim_srv = self.create_service(Empty,
                                           'simulate',
                                           self.sim_callback,
                                           callback_group=MutuallyExclusiveCallbackGroup())
        self.real_putt_srv = self.create_service(Empty,
                                                 'real_putt',
    ...
    def update_real_world_positions(self):
        try:
            ...
            # hole_tf = self.tf_buffer.lookup_transform(self.base_frame,
            # self.hole_tag_frame, rclpy.time.Time())
            ...
            # ])
            #
            # ball_tf = self.tf_buffer.lookup_transform(self.base_frame,
            # self.ball_tag_frame, rclpy.time.Time())
            ...
            base_cam_tf = self.tf_buffer.lookup_transform(self.base_frame,
                                                          self.camera_frame,
                                                          rclpy.time.Time())
            cam_hole_tf = self.tf_buffer.lookup_transform(self.camera_frame,
                                                          self.hole_tag_frame,
                                                          rclpy.time.Time())
            cam_ball_tf = self.tf_buffer.lookup_transform(self.camera_frame,
                                                          self.ball_tag_frame,
                                                          rclpy.time.Time())
            ...
            self.get_logger().info(f'Real world positions updated: Ball at {self.ball_position},',
                                   'Hole at {self.hole_position}')
    ...
    async def real_putt_callback(self, request, response):
        if self.use_simulation_mode:
            self.get_logger().info('Real putt requested, but simulation_mode is True.',
                                   'Switch to False.')
            return response
    ...
    def animate_ball_movement(self, unit_direction, distance):
        ...
        self.ball_animation_timer = self.create_timer(self.ball_animation_dt,
