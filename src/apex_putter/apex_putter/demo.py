...
class DemoNode(Node):
    def __init__(self):
        ...
        # Services
        self.ready_srv = self.create_service(Empty,
                                             'ready',
                                             self.ready_callback,
                                             callback_group=MutuallyExclusiveCallbackGroup())
        self.home_srv = self.create_service(Empty,
                                            'home_robot',
                                            self.home_callback,
                                            callback_group=MutuallyExclusiveCallbackGroup())
        self.putt_srv = self.create_service(Empty,
                                            'putt',
                                            self.putt_callback,
                                            callback_group=MutuallyExclusiveCallbackGroup())
        self.swing_srv = self.create_service(Empty,
                                             'swing',
                                             self.swing_callback,
                                             callback_group=MutuallyExclusiveCallbackGroup())
    ...
    async def home_callback(self, request, response):
        ...
        await self.MPI.move_arm_joints(joint_values=[-0.4, -0.4, 0.0, -1.6, 0.0, 1.57, 0.0],
    ...
    async def ready_callback(self, request, response):
        ...
        # await self.MPI.add_box('ball', (0.042, 0.042, 0.042),
        # (ball_tf.pose.position.x, ball_tf.pose.position.y, ball_tf.pose.position.z))
        await self.MPI.move_arm_pose(ideal_pose,
                                     max_velocity_scaling_factor=0.2,
                                     max_acceleration_scaling_factor=0.2)
    ...
    async def putt_callback(self, request, response):
        ...
        self.get_logger().info('=============================================================')
        ideal_ee_transform = self.tf_buffer.lookup_transform(self.base_frame,
                                                             'goal_ee',
                                                             rclpy.time.Time())
        ideal_pose = Pose()
        ...
        await self.MPI.move_arm_pose(putt_pose_2,
    ...
    async def swing_callback(self, request, response):
        ...
        # Swing the putter
        await self.MPI.move_arm_joints(joint_values=swung_joint_values,
